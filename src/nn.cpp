...
using Vectori32 = __m128i;
    ...
    #endif
i32 ValueNetwork::vectorizedSCReLU(const Accumulator& stm, const Accumulator& nstm, usize bucket) {
...
usize ValueNetwork::feature(Color perspective, Color color, PieceType piece, Square square) {
...
void ValueNetwork::loadNetwork(const string& filepath) {
...
int ValueNetwork::forwardPass(const Board* board, const AccumulatorPair& accumulators) {
    ...
    if constexpr (ACTIVATION != SCReLU) {
        for (usize i = 0; i < HL_SIZE; i++) {
            ...
            if constexpr (ACTIVATION == ReLU)
                eval += NN::ReLU(accumulatorSTM[i]) * weightsToOut[outputBucket][i];
            if constexpr (ACTIVATION == CReLU)
                eval += NN::CReLU(accumulatorSTM[i]) * weightsToOut[outputBucket][i];

            // Last HL_SIZE weights are for OPP
            if constexpr (ACTIVATION == ReLU)
                eval += NN::ReLU(accumulatorOPP[i]) * weightsToOut[outputBucket][HL_SIZE + i];
            if constexpr (ACTIVATION == CReLU)
                eval += NN::CReLU(accumulatorOPP[i]) * weightsToOut[outputBucket][HL_SIZE + i];
        }
    }
    else
...
void ValueNetwork::showBuckets(const Board* board, const AccumulatorPair& accumulators) {
    const usize divisor     = 32 / OUTPUT_BUCKETS;
...
double ValueNetwork::evaluate(const Board& board) {
    AccumulatorPair accumulators(board);
    return cpToWDL(forwardPass(&board, accumulators));
}